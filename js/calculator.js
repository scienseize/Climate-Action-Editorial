...
const EMISSION_FACTORS = {
  ...
  jeepney:    0.10,    // traditional diesel jeepney, per passenger
...
function createLegRow(mode = '', distanceKm = '') {
  ...
  const legIndex = container ? container.querySelectorAll('.leg-row').length + 1 : 1;
  ...
  select.className = 'leg-mode flex-1 bg-white border-2 border-gray-200 focus:border-primary focus:outline-none text-gray-900 rounded-xl px-3 py-2.5 text-sm transition-colors';
  ...
  distInput.className = 'leg-distance w-24 bg-white border-2 border-gray-200 focus:border-primary focus:outline-none text-gray-900 rounded-xl px-3 py-2.5 text-sm transition-colors';
  distInput.min = '0';
  distInput.max = '500';
  distInput.step = '0.5';
  ...
  removeBtn.className = 'leg-remove w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-secondary hover:bg-red-50 transition-colors shrink-0';
...
function updateLivePanel() {
  ...
  if (legs.length === 0) {
    ...
    if (dangerEl)    { dangerEl.textContent = '—'; dangerEl.setAttribute('data-level', 'none'); }
    if (fill)        { fill.setAttribute('data-level', 'none'); fill.style.setProperty('--fill-width', '0%'); }
    if (pctLabel)    pctLabel.textContent = '0%';
    return;
  }

  const dailyKg  = legs.reduce((s, l) => s + EMISSION_FACTORS[l.mode] * l.distanceKm * 2, 0);
  ...
  if (transportEl) {
    transportEl.textContent = legCount === 1
      ? (MODE_LABELS[legs[0].mode] || '—')
      : `${legCount}-leg commute`;
  }
  if (dailyEl)  dailyEl.textContent = dailyKg.toFixed(2) + ' kg CO₂';
  if (dangerEl) { dangerEl.textContent = danger.label; dangerEl.setAttribute('data-level', danger.label); }
  if (fill)     { fill.setAttribute('data-level', danger.label); fill.style.setProperty('--fill-width', width + '%'); }
...
function validateForm() {
  ...
  if (validLegs.length === 0) errors.push('Please add at least one commute leg with a mode and distance.');
...
function handleSubmit(e) {
  ...
  const primaryLeg = legs.reduce((max, l) =>
    (EMISSION_FACTORS[l.mode] * l.distanceKm) > (EMISSION_FACTORS[max.mode] * max.distanceKm) ? l : max
  );
